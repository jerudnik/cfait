...
|---|---|---|
| `vendor/upstream` | Exact mirror of `trougnouf/cfait` primary branch | None |
...
```sh
gh workflow run "Fork maintenance" --repo jerudnik/cfait -f task=sync-upstream
```

The workflow sets `vendor/upstream` to `upstream/master`, rebases
...
```mermaid
flowchart LR
  U["upstream/master<br/>trougnouf/cfait"] -->|"6h scheduler"| CI
  CI["sync-upstream:<br/>ff vendor/upstream,<br/>rebase distro/nix, then main"] --> GH["jerudnik/cfait:<br/>vendor/upstream<br/>distro/nix<br/>main"]
  GH -->|"nix.yml build matrix"| CX["Cachix:<br/>jerudnik-cfait.cachix.org"]
  GH -->|"fork-sync"| L["local clone"]
  GH -->|"nix flake update cfait"| FX["consumer flake"]
  CX -.->|"substituter"| FX
  CX -.->|"substituter"| L
```
...
|---|---|---|
| `upstream/master` | upstream maintainers | source of truth for upstream code |
| `jerudnik/cfait` rails | `Fork maintenance` CI every 6h | yes (only writer) |
| Cachix `jerudnik-cfait` | `nix.yml` build matrix on push | prebuilt artifact cache |
| local clone | `fork-sync` | read-only mirror of the fork |
| consumer `cfait` input | `nix flake update cfait` | consumer pin |

## Updating the consumer pin

A consumer flake pins `cfait.url = "github:jerudnik/cfait/main"`. Its
`flake.lock` only moves when bumped:

```sh
nix flake update cfait   # repin to the latest reconciled tip
nix flake check            # or the repo's validation gate
git commit -m "chore(flake): bump cfait" flake.lock
```
