...
fun formatGoalDuration(currentMins: Int, targetMins: Int): Pair<String, String> {
    val format = { m: Int ->
        when {
            m > 0 && m % 1440 == 0 -> "${m / 1440}d"
            m > 0 && m % 60 == 0 -> "${m / 60}h"
            else -> "${m}m"
        }
    }
    val formatFloat = { m: Int ->
        when {
            targetMins > 0 && targetMins % 1440 == 0 -> String.format(java.util.Locale.US, "%.1fd", m / 1440f).replace(".0d", "d")
            targetMins > 0 && targetMins % 60 == 0 -> String.format(java.util.Locale.US, "%.1fh", m / 60f).replace(".0h", "h")
            else -> "${m}m"
        }
    }

    val tStr = format(targetMins)
    val cStr = formatFloat(currentMins)

    val tSuffix = tStr.filter { it.isLetter() }
    val cSuffix = cStr.filter { it.isLetter() }

    return if (cSuffix == tSuffix && cSuffix.isNotEmpty()) {
        Pair(cStr.removeSuffix(cSuffix), tStr)
...
fun calculateGoalProgress(
    key: String,
    goal: com.trougnouf.cfait.core.MobileGoal,
    tasks: List<com.trougnouf.cfait.core.MobileTask>,
    countSessions: Boolean,
    defaultDur: Int
): Int {
    val now = java.time.ZonedDateTime.now(java.time.ZoneId.systemDefault())
    val startOfPeriod = when (goal.period) {
        com.trougnouf.cfait.core.MobileGoalPeriod.DAILY -> now.truncatedTo(java.time.temporal.ChronoUnit.DAYS)
        com.trougnouf.cfait.core.MobileGoalPeriod.WEEKLY -> now.with(java.time.temporal.TemporalAdjusters.previousOrSame(java.time.DayOfWeek.MONDAY)).truncatedTo(java.time.temporal.ChronoUnit.DAYS)
        com.trougnouf.cfait.core.MobileGoalPeriod.MONTHLY -> now.withDayOfMonth(1).truncatedTo(java.time.temporal.ChronoUnit.DAYS)
        com.trougnouf.cfait.core.MobileGoalPeriod.QUARTERLY -> {
            val qMonth = ((now.monthValue - 1) / 3) * 3 + 1
            now.withMonth(qMonth).withDayOfMonth(1).truncatedTo(java.time.temporal.ChronoUnit.DAYS)
        }
        com.trougnouf.cfait.core.MobileGoalPeriod.HALF_YEARLY -> {
            val hMonth = ((now.monthValue - 1) / 6) * 6 + 1
            now.withMonth(hMonth).withDayOfMonth(1).truncatedTo(java.time.temporal.ChronoUnit.DAYS)
        }
        com.trougnouf.cfait.core.MobileGoalPeriod.YEARLY -> now.withDayOfYear(1).truncatedTo(java.time.temporal.ChronoUnit.DAYS)
    }

    val endOfPeriod = when (goal.period) {
        com.trougnouf.cfait.core.MobileGoalPeriod.DAILY -> startOfPeriod.plusDays(1)
        com.trougnouf.cfait.core.MobileGoalPeriod.WEEKLY -> startOfPeriod.plusDays(7)
        com.trougnouf.cfait.core.MobileGoalPeriod.MONTHLY -> startOfPeriod.plusMonths(1)
        com.trougnouf.cfait.core.MobileGoalPeriod.QUARTERLY -> startOfPeriod.plusMonths(3)
        com.trougnouf.cfait.core.MobileGoalPeriod.HALF_YEARLY -> startOfPeriod.plusMonths(6)
        com.trougnouf.cfait.core.MobileGoalPeriod.YEARLY -> startOfPeriod.plusYears(1)
    }

    val startTs = startOfPeriod.toEpochSecond()
    val endTs = endOfPeriod.toEpochSecond()
    var progress = 0

    val isTag = key.startsWith("#")
    val cleanKey = if (isTag) key.removePrefix("#") else if (key.startsWith("@@")) key.removePrefix("@@") else key

    for (t in tasks) {
        val matches = if (isTag) {
            t.categories.any { it == cleanKey || it.startsWith("$cleanKey:") }
        } else {
            t.location == cleanKey || t.location?.startsWith("$cleanKey:") == true
        }

        if (!matches) continue

        if (goal.goalType == com.trougnouf.cfait.core.MobileGoalType.COUNT) {
            var taskProgress = 0
            if (countSessions) {
                for (s in t.sessions) {
                    if (s.endMs / 1000 >= startTs && s.startMs / 1000 < endTs) {
                        taskProgress += 1
                    }
                }
            }
            if (t.isDone && t.completedDateIso != null) {
                try {
                    val compTs = java.time.OffsetDateTime.parse(t.completedDateIso).toEpochSecond()
                    if (compTs in startTs until endTs && taskProgress == 0) {
                        taskProgress += 1
                    }
                } catch (e: Exception) {}
            }
            progress += taskProgress
        } else if (goal.goalType == com.trougnouf.cfait.core.MobileGoalType.DURATION) {
            var taskTime = 0
            for (s in t.sessions) {
                val sStart = s.startMs / 1000
                val sEnd = s.endMs / 1000
                if (sEnd >= startTs && sStart < endTs) {
                    val overlapStart = maxOf(sStart, startTs)
                    val overlapEnd = minOf(sEnd, endTs)
                    if (overlapEnd > overlapStart) {
                        taskTime += ((overlapEnd - overlapStart) / 60).toInt()
                    }
                }
            }
            if (t.lastStartedAt != null) {
                val currentTs = minOf(System.currentTimeMillis() / 1000, endTs)
                if (currentTs > startTs) {
                    val overlapStart = maxOf(t.lastStartedAt!!, startTs)
                    if (currentTs > overlapStart) {
                        taskTime += ((currentTs - overlapStart) / 60).toInt()
                    }
                }
            }
            if (t.isDone && t.completedDateIso != null) {
                try {
                    val compTs = java.time.OffsetDateTime.parse(t.completedDateIso).toEpochSecond()
                    if (compTs in startTs until endTs) {
                        val totalTracked = (t.timeSpentSeconds / 60u).toInt()
                        val est = t.durationMins?.toInt() ?: defaultDur
                        if (est > totalTracked) {
                            taskTime += est - totalTracked
                        }
                    }
                } catch (e: Exception) {}
            }
